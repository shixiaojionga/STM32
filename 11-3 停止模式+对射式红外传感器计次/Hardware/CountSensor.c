...
void CountSensor_Init(void)
{
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);//配置GPIOB8端口
	
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB, GPIO_PinSource14);  //AFIO外部中断引脚选择，选择AFIO8端口
...
{
	if(EXTI_GetITStatus(EXTI_Line14) == SET )//因为9-5通道都可以进来，故要判断是不是想要的8通道进来
	{
		if(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_14) == 1)//下降沿时为0，上升沿时为1，双边沿时为0|1
		{
			CountSensor_Count++;
